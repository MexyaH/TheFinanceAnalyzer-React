...
import {
    Box, FormControl, Text, HStack,
    Textarea, Button, Select, Grid,
    Flex, Input,
...
const addAssistance: React.FC = () => {
    ...
    const [titoloIntervento, setTitoloIntervento] = useState<string>('');
    const [uploadProgressPreventivo, setUploadProgressPreventivo] = useState<number>(0);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        await setDoc(doc(firestore, 'operazioni', id.toString()), formData).then(() => {
            showSuccessToast("Form submitted successfully!");
            navigate('/');
        }).catch(() => {
    ...
    return (
        <div>
            ...
            <Flex justifyContent="center" minHeight="100vh" width={'100%'}>
                <Box width="100%">
                    <Navbar />
                    <Grid templateColumns={["1fr", "80% 20%"]} gap={6} width="100%" p={10}>
                        <Box border="1px solid grey" p={4} borderRadius="md">
                            <form onSubmit={handleSubmit}>
                                ...
                                <Grid templateColumns={["repeat(1, 1fr)", "repeat(2, 1fr)"]} gap={6}>
                                    <Select value={clienteSelected} onChange={(e) => setClienteSelected(e.target.value)} required>
                                        <option value="" disabled>Seleziona un cliente</option>
                                        {clientiList.map((cliente) => (
                                            <option key={cliente} value={cliente}>{cliente}</option>
                                        ))}
                                    </Select>
                                    <Input value={titoloIntervento} onChange={(e) => setTitoloIntervento(e.target.value)} placeholder="Lavorazione" type="text" required/>
                                </Grid>
                                ...
                                <Grid templateColumns={["repeat(1, 1fr)", "repeat(3, 1fr)"]} gap={6} mt={8}>
                                    ...
                                    <Select value={oreLavorate} onChange={(e) => setOreLavorate(parseInt(e.target.value))} required>
                                        <option value={0} selected disabled>Ore Lavorate</option>
                                ...
                                <Grid templateColumns={["repeat(1, 1fr)", "repeat(5, 1fr)"]} gap={6} width="100%" mt={8}>
                                    <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome Pezzo" type="text" />
                                    <Select value={qta} onChange={(e) => setQta(parseInt(e.target.value))}>
                                    ...
                                    <Input value={price} onChange={(e) => setPrice(parseFloat(e.target.value))} placeholder="Prezzo" type="number" />
                                    <Input value={fornitor} onChange={(e) => setFornitor(e.target.value)} placeholder="Fornitore" type="text" />
                                    <Button onClick={handleAddPezzo}>+</Button>
                                </Grid>
                                <Grid templateColumns={["repeat(1, 1fr)", "repeat(4, 1fr)"]} gap={6} width="100%" mt={8}>
                                    <Input value={payment} onChange={(e) => setPayment(e.target.value)} placeholder="Tipo di pagamento" type="text" />
                                    <Input value={amount} onChange={(e) => setAmount(parseFloat(e.target.value))} placeholder="Acconto in €" type="number" />
                                    <SingleDatepicker
                                        name="date-input"
                                        date={date}
                                    ...
                                    />
                                    <Button onClick={handleAddAcconto}>+</Button>
                                </Grid>
                                <Grid templateColumns={["repeat(1, 1fr)", "repeat(1, 1fr)"]} gap={6} mt={8}>
                                    <Input value={tipoMotore} onChange={(e) => setTipoMotore(e.target.value)} placeholder="Tipo Motore"/>
                                ...
                                <Grid templateColumns={["1fr"]} gap={6} width="100%">
                                    <FormControl id="file" mb={3} width="100%">
                                        ...
                                        <HStack spacing={4} width="100%">
                                            <Input alignContent="center" flex="1" key={inputKey} type="file" onChange={handleFileChange()} />
                                            <Button flexShrink="0" onClick={handleUploadPreventivo} /*style={{ background: isClickedSchema ? 'red' : '#2c323d' }}*/>
                                                Upload
                                            </Button>
                                            {isUploading &&
                                                <CircularProgress value={uploadProgressPreventivo} thickness='4px'>
                                                    <CircularProgressLabel>{uploadProgressPreventivo}%</CircularProgressLabel>
                                                </CircularProgress>
                                            }
                                        </HStack>
